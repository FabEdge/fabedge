...
func (e *Entry) Validate(set *IPSet) bool {
	...
	case HashIP:
		if ip := net.ParseIP(e.IP); ip == nil {
			err := fmt.Errorf("%s is not a valid textual representation of an IP address", e.IP)
			klog.Errorf("Failed to parse entry %v ip %v for ipset %v, error: %v", e, e.IP, set, err)
			return false
		}
	case HashNet:
		if _, _, err := net.ParseCIDR(e.Net); err != nil {
			klog.Errorf("Failed to parse entry %v net %v for ipset %v, error: %v", e, e.Net, set, err)
			return false
		}
...
func (e *Entry) String() string {
	...
	case HashIP:
		...
		return e.IP
	case HashNet:
		// Entry{10.10.0.0/16} -> 10.10.0.0/16
		return e.Net
	}
	return ""
}
