...
const (
	...
	// HashIP represents the `hash:ip` type ipset. The  hash:ip  set type uses a hash to store IP host addresses (default) or network addresses.
	// Zero valued IP address cannot be stored in a hash:ip type of set.
	HashIP Type = "hash:ip"
	// HashNet represents the `hash:Net` type ipset. The hash:net set type uses a hash to store different sized IP network addresses.  Network ad‐
