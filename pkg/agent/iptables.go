...
func (m *Manager) ensureIPTablesRules() error {
	...
	configs := []struct {
		...
		{
			ipt:       m.ipt4,
			ipset:     &ipset.IPSet{Name: IPSetFabEdgePeerCIDR, SetType: ipset.HashNet, HashFamily: ipset.ProtocolFamilyIPV4},
			peerIPSet: peerIPSet4,
			subnets:   subnetsIP4,
		},
		{
			ipt:       m.ipt6,
			ipset:     &ipset.IPSet{Name: IPSetFabEdgePeerCIDR6, SetType: ipset.HashNet, HashFamily: ipset.ProtocolFamilyIPV6},
			peerIPSet: peerIPSet6,
			subnets:   subnetsIP6,
		},
	}
	...
	for _, c := range configs {
		...
		if m.MASQOutgoing {
			if err := m.configureOutboundRules(c.ipt, c.ipset, c.subnets, c.peerIPSet, clearOutgoingChain); err != nil {
				return err
			}
		}
	}
...
func (m *Manager) configureOutboundRules(ipt *iptables.IPTables, set *ipset.IPSet, subnets []string, peerIPSet sets.String, clearFabEdgeNatOutgoingChain bool) error {
	...
	for _, subnet := range subnets {
		...
		ensureRule := ipt.AppendUnique
		if err := ensureRule(TableNat, ChainFabEdgeNatOutgoing, "-s", subnet, "-m", "set", "--match-set", set.Name, "dst", "-j", "RETURN"); err != nil {
			m.log.Error(err, "failed to append rule", "table", TableNat, "chain", ChainFabEdgeNatOutgoing, "rule", fmt.Sprintf("-s %s -m set --match-set %s dst -j RETURN", subnet, set.Name))
			continue
		}
	...
	if err := m.ipset.EnsureIPSet(set, peerIPSet); err != nil {
		m.log.Error(err, "failed to sync ipset", "ipsetName", set.Name)
		return err
	}
